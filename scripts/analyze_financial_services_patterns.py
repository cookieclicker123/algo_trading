#!/usr/bin/env python3
"""
Comprehensive Financial Services Sector Pattern Analysis.
...
def is_financial_services(record: Dict[str, Any]) -> bool:
    """Check if record is for a Financial Services sector stock."""
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    financial_keywords = [
        # Banking & Lending
        'loan', 'loans', 'lending', 'lender', 'credit', 'debt', 'borrow', 'borrowing',
        'mortgage', 'mortgages', 'refinance', 'refinancing', 'interest rate', 'rates',
        'deposit', 'deposits', 'savings', 'checking', 'account', 'accounts',
        
        # Investment & Trading
        'investment', 'investments', 'invest', 'investor', 'investors', 'portfolio',
        'trading', 'trade', 'trader', 'traders', 'market', 'markets', 'exchange',
        'brokerage', 'broker', 'brokers', 'advisory', 'adviser', 'advisor',
        
        # M&A & Deals
        'acquisition', 'acquire', 'acquires', 'merger', 'merge', 'merges',
        'deal', 'deals', 'transaction', 'transactions', 'agreement', 'agreements',
        'partnership', 'partnerships', 'alliance', 'alliances', 'joint venture',
        'takeover', 'buyout', 'buyout', 'purchase', 'purchases',
        
        # Insurance
        'insurance', 'insurer', 'insurers', 'policy', 'policies', 'premium', 'premiums',
        'coverage', 'claim', 'claims', 'underwriting', 'underwrite',
        ...
        'return', 'returns', 'roi', 'roe', 'roa',
        
        # Regulatory & Compliance
        'regulatory', 'regulation', 'regulations', 'approval', 'approvals', 'approved',
        'license', 'licenses', 'licensed', 'permit', 'permits', 'compliance',
        'sec', 'federal reserve', 'fed', 'fcc', 'fdic',
        
        # Growth & Expansion
        'growth', 'grow', 'grows', 'expanding', 'expansion', 'expand', 'expands',
        'launch', 'launches', 'launched', 'introduce', 'introduces', 'introduction',
        'new', 'announce', 'announces', 'announcement', 'announcements',
        
        # Positive Signals
        'positive', 'strong', 'stronger', 'strength', 'improve', 'improves', 'improvement',
        'increase', 'increases', 'increased', 'rise', 'rises', 'rising', 'up',
        'success', 'successful', 'win', 'wins', 'gain', 'gains', 'gained',
        'record', 'records', 'high', 'higher', 'highest', 'best', 'better',
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in financial_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in financial_keywords):
...
def main():
    ...
    print(f"   Financial Services records found: {len(financial_services_records)}")
    ...
    for record in financial_services_records:
    ...
    print(f"   Financial Services winners (>5% gain): {len(winners)}")
    print(f"   Financial Services non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Financial Services losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_financial_services_records': len(financial_services_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "financial_services_pattern_analysis.json"
