#!/usr/bin/env python3
"""
Comprehensive Industrials Sector Pattern Analysis.
...
def is_industrials(record: Dict[str, Any]) -> bool:
    """Check if record is for an Industrials sector stock."""
    ...
    for ticker, metadata in ticker_metadata.items():
        if isinstance(metadata, dict):
            ...
            if 'industrial' in sector_lower or 'industrials' in sector_lower:
                return True
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    # Industrials keywords
    keywords = []
    industrials_keywords = [
        # Manufacturing & Production
        'manufacturing', 'manufacture', 'production', 'produce', 'produces',
        'factory', 'factories', 'plant', 'plants', 'facility', 'facilities',
        'assembly', 'assembly line', 'production line',
        
        # Construction & Infrastructure
        'construction', 'build', 'builds', 'building', 'infrastructure',
        'project', 'projects', 'contract', 'contracts', 'contractor', 'contractors',
        'development', 'develop', 'develops', 'developing',
        
        # Machinery & Equipment
        'machinery', 'machine', 'machines', 'equipment', 'equipments',
        'tool', 'tools', 'automation', 'automated', 'robotic', 'robotics',
        'industrial', 'industrial equipment',
        
        # Transportation & Logistics
        'transportation', 'transport', 'logistics', 'shipping', 'ship',
        'freight', 'cargo', 'delivery', 'deliveries', 'supply chain',
        'truck', 'trucks', 'fleet', 'fleets', 'vehicle', 'vehicles',
        'railroad', 'rail', 'railway', 'aviation', 'airline', 'airlines',
        
        # Aerospace & Defense
        'aerospace', 'defense', 'defence', 'military', 'aircraft',
        'airplane', 'airplanes', 'helicopter', 'helicopters', 'drone', 'drones',
        'satellite', 'satellites', 'space', 'rocket', 'rockets',
        
        # Energy & Power
        'energy', 'power', 'electric', 'electrical', 'solar', 'wind',
        'renewable', 'renewables', 'battery', 'batteries', 'grid',
        'oil', 'gas', 'natural gas', 'petroleum', 'refinery', 'refineries',
        ...
        # Contracts & Orders
        'order', 'orders', 'booking', 'bookings', 'award', 'awards', 'awarded',
        ...
        'delay', 'delays', 'delayed', 'shutdown', 'shutdowns'
    ]
    
    for keyword in industrials_keywords:
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in industrials_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in industrials_keywords):
...
def main():
    ...
    print(f"   Industrials records found: {len(industrials_records)}")
    ...
    for record in industrials_records:
    ...
    print(f"   Industrials winners (>5% gain): {len(winners)}")
    print(f"   Industrials non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Industrials losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_industrials_records': len(industrials_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "industrials_pattern_analysis.json"
