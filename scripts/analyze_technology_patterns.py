#!/usr/bin/env python3
"""
Comprehensive Technology Sector Pattern Analysis.
...
def is_technology(record: Dict[str, Any]) -> bool:
    """Check if record is for a Technology sector stock."""
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    technology_keywords = [
        ...
        'neural network', 'neural networks', 'llm', 'llms', 'gpt', 'chatgpt',
        'generative', 'generative ai', 'automation', 'automated',
        
        # Software & Platforms
        'software', 'platform', 'platforms', 'saas', 'cloud', 'cloud computing',
        'api', 'apis', 'integration', 'integrations', 'sdk', 'framework',
        'application', 'applications', 'app', 'apps', 'service', 'services',
        
        # Hardware & Devices
        'chip', 'chips', 'semiconductor', 'semiconductors', 'processor', 'processors',
        'gpu', 'gpus', 'cpu', 'cpus', 'hardware', 'device', 'devices',
        'smartphone', 'smartphones', 'tablet', 'tablets', 'laptop', 'laptops',
        
        # Data & Analytics
        'data', 'analytics', 'analytics', 'big data', 'database', 'databases',
        'algorithm', 'algorithms', 'insights', 'intelligence',
        
        # Security & Blockchain
        'security', 'cybersecurity', 'encryption', 'blockchain', 'crypto', 'cryptocurrency',
        'bitcoin', 'ethereum', 'nft', 'nfts', 'web3', 'defi',
        ...
        'breakthrough', 'innovative', 'innovation', 'revolutionary',
        
        # Gaming & Entertainment
        'game', 'games', 'gaming', 'gamer', 'gamers', 'esports',
        'streaming', 'stream', 'content', 'creator', 'creators',
        
        # Social & Communication
        'social', 'media', 'network', 'networks', 'messaging', 'communication',
        'connect', 'connects', 'connectivity', 'wireless', '5g', '6g',
        ...
        'breach', 'breaches', 'hack', 'hacked', 'vulnerability', 'vulnerabilities'
    ]
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in technology_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in technology_keywords):
...
def main():
    ...
    print(f"   Technology records found: {len(technology_records)}")
    ...
    print(f"   Technology winners (>5% gain): {len(winners)}")
    print(f"   Technology non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Technology losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_technology_records': len(technology_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "technology_pattern_analysis.json"
