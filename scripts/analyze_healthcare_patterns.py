#!/usr/bin/env python3
"""
Comprehensive Healthcare Sector Pattern Analysis.
...
def is_healthcare(record: Dict[str, Any]) -> bool:
    """Check if record is for a healthcare sector stock."""
    ...
    for ticker, metadata in ticker_metadata.items():
        if isinstance(metadata, dict):
            ...
            if 'healthcare' in sector_lower or 'health' in sector_lower:
                return True
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    keywords = []
    healthcare_keywords = [
        'fda', 'approval', 'approved', 'approves', 'clearance', 'authorized',
        'phase', 'trial', 'clinical', 'study', 'studies', 'data', 'results',
        'drug', 'treatment', 'therapy', 'therapeutic', 'therapeutics',
        'breakthrough', 'efficacy', 'safety', 'efficacious', 'safe',
        'submission', 'nda', 'bla', 'pdufa', 'catalyst', 'update',
        'orphan', 'designation', 'fast track', 'breakthrough therapy',
        'meeting', 'conference', 'presentation', 'announces', 'announcement',
        'partnership', 'collaboration', 'launch', 'launches', 'launched',
        'acquisition', 'merger', 'deal', 'agreement', 'contract',
        'revenue', 'sales', 'earnings', 'profit', 'guidance',
        'positive', 'successful', 'favorable', 'encouraging',
        'disease', 'indication', 'patient', 'patients', 'diagnosis'
    ]
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in healthcare_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in healthcare_keywords):
...
def main():
    ...
    print(f"   Healthcare records found: {len(healthcare_records)}")
    ...
    print(f"   Healthcare winners (>5% gain): {len(winners)}")
    print(f"   Healthcare non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Healthcare losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_healthcare_records': len(healthcare_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "healthcare_pattern_analysis.json"
