#!/usr/bin/env python3
"""
Comprehensive Basic Materials Sector Pattern Analysis.
...
def is_basic_materials(record: Dict[str, Any]) -> bool:
    """Check if record is for a Basic Materials sector stock."""
    ...
    for ticker, metadata in ticker_metadata.items():
        if isinstance(metadata, dict):
            ...
            if 'basic materials' in sector_lower or 'materials' in sector_lower:
                return True
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    # Basic Materials keywords
    keywords = []
    basic_materials_keywords = [
        # Metals & Mining
        'metal', 'metals', 'mining', 'mine', 'mines', 'miner', 'miners',
        'gold', 'silver', 'copper', 'iron', 'steel', 'aluminum', 'aluminium',
        'ore', 'ores', 'deposit', 'deposits', 'reserve', 'reserves',
        'extraction', 'extract', 'extracts', 'drilling', 'drill', 'drills',
        
        # Chemicals
        'chemical', 'chemicals', 'petrochemical', 'petrochemicals',
        'polymer', 'polymers', 'plastics', 'plastic',
        'fertilizer', 'fertilizers', 'pesticide', 'pesticides',
        
        # Energy Materials
        'oil', 'gas', 'natural gas', 'petroleum', 'crude', 'refinery', 'refineries',
        'energy', 'fuel', 'fuels', 'coal', 'uranium',
        
        # Construction Materials
        'cement', 'concrete', 'aggregate', 'aggregates', 'lumber', 'timber',
        'construction', 'building materials', 'materials',
        
        # Paper & Packaging
        'paper', 'pulp', 'packaging', 'packaging materials',
        'container', 'containers', 'box', 'boxes',
        ...
        'production', 'produce', 'produces', 'manufacturing', 'manufacture',
        'facility', 'facilities', 'plant', 'plants', 'factory', 'factories',
        'capacity', 'capabilities', 'operation', 'operations', 'operate', 'operates',
        
        # Contracts & Orders
        'contract', 'contracts', 'order', 'orders', 'booking', 'bookings',
        'award', 'awards', 'awarded', 'agreement', 'agreements',
    ...
    for keyword in basic_materials_keywords:
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in basic_materials_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in basic_materials_keywords):
...
def main():
    ...
    print(f"   Basic Materials records found: {len(basic_materials_records)}")
    ...
    for record in basic_materials_records:
    ...
    print(f"   Basic Materials winners (>5% gain): {len(winners)}")
    print(f"   Basic Materials non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Basic Materials losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_basic_materials_records': len(basic_materials_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "basic_materials_pattern_analysis.json"
