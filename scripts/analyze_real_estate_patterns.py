#!/usr/bin/env python3
"""
Comprehensive Real Estate Sector Pattern Analysis.
...
def is_real_estate(record: Dict[str, Any]) -> bool:
    """Check if record is for a Real Estate sector stock."""
    ...
    for ticker, metadata in ticker_metadata.items():
        if isinstance(metadata, dict):
            ...
            if 'real estate' in sector_lower or 'realestate' in sector_lower:
                return True
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    real_estate_keywords = [
        # Property Types
        'property', 'properties', 'real estate', 'realestate', 'reit', 'reits',
        'commercial', 'residential', 'retail', 'office', 'industrial', 'warehouse', 'warehouses',
        'apartment', 'apartments', 'condo', 'condos', 'condominium', 'condominiums',
        'hotel', 'hotels', 'hospitality', 'resort', 'resorts',
        'land', 'land development', 'development', 'develop', 'develops', 'developing',
        
        # Transactions & Deals
        'acquisition', 'acquire', 'acquires', 'merger', 'merge', 'merges',
        'deal', 'deals', 'transaction', 'transactions', 'agreement', 'agreements',
        'sale', 'sales', 'sell', 'sells', 'sold', 'purchase', 'purchases', 'buy', 'buys', 'bought',
        'lease', 'leases', 'leasing', 'rent', 'rents', 'rental', 'rentals',
        
        # Financing & Investment
        'financing', 'finance', 'finances', 'loan', 'loans', 'mortgage', 'mortgages',
        'refinance', 'refinancing', 'refinanced', 'credit', 'debt',
        'investment', 'invest', 'invests', 'investor', 'investors', 'portfolio', 'portfolios',
        'fund', 'funds', 'funding', 'capital', 'equity',
        
        # Development & Construction
        'construction', 'build', 'builds', 'building', 'buildings', 'infrastructure',
        'project', 'projects', 'contract', 'contracts', 'contractor', 'contractors',
        'renovation', 'renovations', 'renovate', 'renovates', 'renovated',
        'expansion', 'expand', 'expands', 'expanding',
        ...
        'management', 'manage', 'manages', 'operate', 'operates', 'operation', 'operations',
        'tenant', 'tenants', 'occupancy', 'occupancy rate', 'vacancy', 'vacancy rate',
        'maintenance', 'maintain', 'maintains',
        
        # Earnings & Financials
        'revenue', 'revenues', 'earnings', 'profit', 'profits', 'profitability',
        'income', 'guidance', 'forecast', 'forecasts', 'outlook', 'expectations',
        'quarter', 'quarters', 'q1', 'q2', 'q3', 'q4', 'fiscal', 'annual',
        'beat', 'beats', 'miss', 'misses', 'exceed', 'exceeds', 'surpass',
        'dividend', 'dividends', 'yield', 'yields', 'payout', 'distribution',
        
        # Growth & Expansion
        'growth', 'grow', 'grows', 'scaling', 'scalable', 'market', 'markets', 'global',
        'capacity', 'capabilities', 'increase', 'increases', 'increased',
        'new', 'launch', 'launches', 'launched', 'introduce', 'introduces',
        
        # Location & Geography
        'location', 'locations', 'region', 'regions', 'regional', 'city', 'cities',
        'state', 'states', 'national', 'nationwide', 'global', 'international',
        ...
        'best', 'better', 'breakthrough', 'innovative', 'innovation',
        ...
        'delay', 'delays', 'delayed', 'default', 'defaults', 'foreclosure', 'foreclosures'
    ]
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in real_estate_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in real_estate_keywords):
...
def main():
    ...
    print(f"   Real Estate records found: {len(real_estate_records)}")
    ...
    for record in real_estate_records:
    ...
    print(f"   Real Estate winners (>5% gain): {len(winners)}")
    print(f"   Real Estate non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Real Estate losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_real_estate_records': len(real_estate_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "real_estate_pattern_analysis.json"
