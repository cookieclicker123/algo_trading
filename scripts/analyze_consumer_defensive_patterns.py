#!/usr/bin/env python3
"""
Comprehensive Consumer Defensive Sector Pattern Analysis.
...
def is_consumer_defensive(record: Dict[str, Any]) -> bool:
    """Check if record is for a Consumer Defensive sector stock."""
    ...
    for ticker, metadata in ticker_metadata.items():
        if isinstance(metadata, dict):
            ...
            if 'consumer defensive' in sector_lower or 'defensive' in sector_lower:
                return True
...
def extract_keywords_phrases(title: str) -> Dict[str, List[str]]:
    ...
    # Consumer Defensive keywords
    keywords = []
    consumer_defensive_keywords = [
        # Food & Beverages
        'food', 'foods', 'beverage', 'beverages', 'grocery', 'groceries',
        'packaged', 'packaging', 'snack', 'snacks', 'cereal', 'cereals',
        'organic', 'natural', 'healthy', 'nutrition', 'nutritional',
        'brand', 'brands', 'product', 'products', 'line', 'lines',
        
        # Household & Personal Care
        'household', 'personal care', 'care', 'cleaning', 'clean',
        'soap', 'detergent', 'detergents', 'cosmetic', 'cosmetics',
        'beauty', 'skincare', 'haircare', 'hygiene',
        ...
        'distribution', 'distribute', 'distributes', 'supply chain',
    ...
    for keyword in consumer_defensive_keywords:
    ...
    for i in range(len(words) - 1):
        ...
        if any(kw in bigram for kw in consumer_defensive_keywords):
            phrases.append(bigram)
        if i < len(words) - 2:
            trigram = f"{words[i]} {words[i+1]} {words[i+2]}"
            if any(kw in trigram for kw in consumer_defensive_keywords):
...
def main():
    ...
    print(f"   Consumer Defensive records found: {len(consumer_defensive_records)}")
    ...
    for record in consumer_defensive_records:
    ...
    print(f"   Consumer Defensive winners (>5% gain): {len(winners)}")
    print(f"   Consumer Defensive non-movers (-1% to 1%): {len(non_movers)} (sampling {len(non_movers_sample)})")
    print(f"   Consumer Defensive losers (<-1%): {len(losers)}")
    ...
    report = {
        'analysis_metadata': {
            ...
            'total_consumer_defensive_records': len(consumer_defensive_records)
        },
        'summary': {
    ...
    # Save report
    output_file = PROJECT_ROOT / "consumer_defensive_pattern_analysis.json"
