#!/usr/bin/env python3
"""
Collect catalyst headlines for 5-10% movers only.

Usage:
    arch -arm64 .venv/bin/python scripts/collect_catalysts_5_to_10.py
"""
...
# Paths
INPUT_FILE = Path("tmp/alpaca_movers/5_to_10_pct_winners.csv")
OUTPUT_FILE = Path("tmp/alpaca_movers/5_to_10_pct_with_catalysts.csv")
CHECKPOINT_FILE = Path("tmp/alpaca_movers/checkpoint_5_to_10.json")
...
async def main():
    print("=" * 60)
    print("CATALYST COLLECTION - 5-10% MOVERS")
    print("=" * 60)
    ...
    pbar = tqdm(total=len(movers), initial=start_idx, desc="5-10% movers")
