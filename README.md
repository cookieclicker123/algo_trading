...
NewsFlash is a high-frequency news trading system: it ingests financial newswire articles in real time, classifies them with LLMs, and routes the actionable ones through a stack of liquidity, latency and momentum filters into automated trades on Alpaca (with extended-hours support). It was profitable in a market regime a few months ago — even during the testing phase, running on deliberately small position sizes. The currently-profitable version of the system is kept private to preserve its edge, but it is fundamentally the same system as what's in this repo — the only material difference is a slightly statistically distinct trade signal sitting on top of the same ingestion, classification, filtering and execution pipeline.

Under the hood, the interesting parts are: a five-microservice event-driven core built on a custom fire-and-forget `AsyncEventBus` with per-subscriber error isolation, so a wire-rate burst of news fans out concurrently instead of queueing; a sub-second budget from publication to order — microstructure prefilters (spread, latency, market cap, price) run *before* the LLM is even called, killing the bulk of articles in milliseconds and saving API spend for the headlines that actually matter; a dual-LLM classification stack (Claude Haiku 4.5 for triage across 38 headline types, then a second sector-specific Haiku call across 60+ sector prompts) where high-conviction headline types bypass the second hop entirely; and bespoke order-execution ladders — *mid-then-chase-the-ask* on entry to save half-spread on calm fills while still catching runners, *chase-the-bid* on exit, both with hard price collars. The genuinely novel piece sits underneath the LLM layer: the system prompts are not hand-wavy "what I think a good headline looks like" — they're the *condensed output of a data pipeline*. Tens of thousands of historical recall records (every article we've ever seen, with full microstructure context and forward-return labels) are fed through custom-finetuned random-forest classifiers per sector/catalyst category to surface the linguistic features that actually carry edge — which n-grams, qualifiers, counterparty patterns, and dollar-magnitude framings statistically predict a 5%+ move and which look the same to a human but don't. Those data-derived features are then *distilled into natural-language system prompts* given to Claude, because in practice an LLM generalises across linguistic variance (paraphrases, novel phrasings, mixed framings) far better than a frozen feature-based classifier ever could. So the random forests do the statistics; the LLM does the language. Every threshold elsewhere in the stack — spreads, latency caps, momentum-exhaustion bounds, time-to-peak windows, stop-loss survival curves, per-catalyst position sizing — comes from the same recall corpus (most of which lives in `tmp/` and isn't visible in the repo, but the engines that produce it are in `src/newsflash/shared/statistics/`). On top of all of that sits a backtesting layer: a tick-replay trade simulator that runs candidate trades through the exact live stop-loss + take-profit + trailing-stop logic against real Alpaca tick data, a nightly confusion-matrix job that labels every decision (TP/FP/FN/TN) into a training set, and pattern/optimal-signal search scripts that sweep thousands of filter combinations to surface the best precision/recall cuts per industry — that feedback loop is what re-tunes the prompts, the forests, and the thresholds you see committed here, and is the same loop that produced the slightly statistically-different signal in the private profitable version.

For a concrete sense of what the system actually does in production, see two example artifacts in the repo root:
- [`premarket_example_trading_session.json`](premarket_example_trading_session.json) — a full premarket session log showing every article tracked, which were filtered out and why (spread, latency, market cap, sector LLM skip, etc.), and which made it through to a trade.
- [`daily_analytics_example.json`](daily_analytics_example.json) — the daily post-session analytics rollup: trades taken, outcomes, missed opportunities, and the per-filter attribution that drives the next round of tuning. Both files were generated during the testing phase on small sums, and demonstrate that the system was already profitable at that scale.
